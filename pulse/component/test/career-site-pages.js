(() => {
    ...
    const expandUrlSet = async (urlset) => {
        const urls = [];
        const allowedSubfolders = getAllowedSubfolders(careerSitePagesLang);
        ...
        const promises = urlElements.map(async (url) => {
            ...
            const subfolder = allowedSubfolders.find((f) => loc.includes(f));

            if (subfolder) {
                subfolderCounts[subfolder] = (subfolderCounts[subfolder] || 0) + 1;
                if (subfolderCounts[subfolder] > 10) return;
    ...
    const getAllowedSubfolders = (lang) => {
        if (lang === "de") {
