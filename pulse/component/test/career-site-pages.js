(() => {
    ...
    const expandUrlSet = async (urlset) => {
        ...
        if (careerSitePagesLang === "de") {
            allowedSubfolders = [
                "/berufsfeld/", "/l%c3%a4nderauswahl/", "/besch%c3%a4ftigung/", "/firma/",
                "/stellenbeschreibung/", "/arbeitsort/"
            ];
        } else if (careerSitePagesLang === "fr") {
            allowedSubfolders = [
                "/cat%c3%a9gorie/", "/lieu/", "/emplois/", "/entreprise/",
                "/emploi/", "/lieu-de-travail/"
            ];
        } else if (careerSitePagesLang === "pt-br") {
            allowedSubfolders = [
                "/%c3%a1rea/", "/localiza%c3%a7%c3%a3o/", "/firma/", "/vaga/", "/sub-localização/"
            ];
        } else {
            allowedSubfolders = [
                "/job/", "/location/", "/employment/", "/category/",
    ...
    const convertSitemapToArray = async (url) => {
        ...
        // ✅ Apply the 10-page limit per subdirectory
        ...
        urls.forEach((urlObj) => {
            ...
            if (subfolderBuckets[firstPart].length < 10) {
    ...
    const sitemapUrl = isLangFolder
        ? `${location.origin}/${pathSegments[0]}/sitemap.xml`
